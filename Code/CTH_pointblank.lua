function point_blank_cth()
	Presets.ChanceToHitModifier.Default["PointBlank"].CalcValue = function (self, attacker, target, body_part_def, action, weapon1, weapon2, lof, aim, opportunity_attack, attacker_pos, target_pos)	
		if attacker and IsKindOf(weapon1, "FirearmProperties") then
			...
			--print("scale factor pb range", scale_factor)	

			--print("pb bonus1", pb_bonus)
			
				pb_bonus = cRound(pb_bonus * scale_factor* const.Combat.R_AimMul/100)
		
			
			--print("pb bonus2", pb_bonus)
			...
			if attacker:IsPointBlankRange(target) then
				...
				return true, pb_bonus, T(461622328882, "Point-Blank Range"),  #metaText ~= 0 and metaText
			else
				...
				if dist <= close_range then
                    ...
                    if scaled_bonus > 0 then
                        return true, scaled_bonus, T(974362381782, "Close Range"),  #metaText ~= 0 and metaText
                    end
