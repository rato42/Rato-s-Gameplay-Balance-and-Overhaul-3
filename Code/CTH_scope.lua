...
PlaceObj('ChanceToHitModifier', {
	CalcValue = function (self, attacker, target, body_part_def, action, weapon1, weapon2, lof, aim, opportunity_attack, attacker_pos, target_pos)
		...
		local metaText = {}
		local value = 0
		local pb_dist = 0 * const.SlabSizeX
		...
		if dist > pb_dist then
			...
			elseif weapon1 and weapon1:HasComponent("ScopePenalty1") then
				...
				max_dist = (6 * const.SlabSizeX) + pb_dist		
				max_penalty = -15
...
local t_id_table = {
[926844781722] = "Scoped Close Range",
}
