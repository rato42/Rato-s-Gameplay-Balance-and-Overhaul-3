function Unit:Idle()
    ...
    if self:IsDead() then
        ...
        if g_Combat and self:CanCower() and
            (self.team.side == "neutral" or self:HasStatusEffect("ForceCower")) and
            not g_Combat:ShouldEndCombat() then
    ...
    elseif g_Combat and self.combat_behavior then
        self:SetCommand(self.combat_behavior,
                        table.unpack(self.combat_behavior_params or empty_table))
    ...
    local dummy_orientation_angle = self:GetPosOrientation(pos, nil, self.stance, true,
                                                           can_reposition)
    if not orientation_angle then
        orientation_angle = self.auto_face and dummy_orientation_angle or
                                self:GetPosOrientation(pos, nil, self.stance, false, can_reposition)
    end
    ...
    if self.aim_action_id and not HasCombatActionInProgress(self) and
        not (self:HasStatusEffect("shooting_stance")) then
        ----------------------
    ...
    if self:ShouldBeIdle() then
        ...
        if anim_style then
            ...
            if self:GetAnimPhase(1) == 0 or self:IsAnimEnd() or
                not IsAnimVariant(self:GetStateText(), base_idle) then
