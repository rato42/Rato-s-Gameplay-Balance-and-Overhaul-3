...
PlaceObj('ChanceToHitModifier', {
    CalcValue = function(self, attacker, target, body_part_def, action, weapon1, weapon2, lof, aim, opportunity_attack, attacker_pos, target_pos)
        ...
        -- if (side == 'player1' or side == 'player2') and isShootingAction(action.id) and aim == 0 and attacker.combat_behavior ~= 'OverwatchAction' then
			...
			if action and action.id == "AutoFire" and aim > 0 then
				return false, 0
			end
			...
			if action.id == "BurstFire" then
				...
				--print(str)
				local reflex = (attacker.Dexterity + attacker.Agility)/2.0
				ratio = str/reflex
				--print("ratio hipfire", ratio)
			end
			
			if not opportunity_attack and aim < 1 and ( (action.id == "BuckshotBurst" or action.id == "MGBurstFire" or action.id == "GrizzlyPerk" or action.id == "AutoFire") or (ratio and ratio > 1)) then
				local reflex = attacker.Strength
			...
			if aim > 0 and aim < 3 then
				max_dist = 26 * const.SlabSizeX
				max_penal = const.Combat.R_SnapshotPenalty
