...
function OnMsg.UnitStanceChanged(unit)
	...
	for _, attach in ipairs(attaches) do
		...
		if parts.Grip and parts.Grip:HasState("folded") then
...
function OnMsg.OnAttack(self, action, target, results, attack_args)
	...
	if const.Combat.R_RecoilP > 0 then
		...
		if IsKindOf(weapon,"HeavyWeapon") then
			...
			local aim_cost = cRoundDown(-recoil_value/45)/2.0  ---51
...
function OnMsg.CombatActionEnd(unit)
	...
	if unit.action_command == "RunAndGun" or unit.action_command == "RecklessAssault" or unit.action_command == "HundredKnives" then
...
function OnMsg.UnitDied(unit)
    if unit then
		...
		local chance = const.BaseDropChance.Ammo +2
