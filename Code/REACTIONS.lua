function OnMsg.UnitStanceChanged(unit)
    ...
    if not (side == 'player1' or side == 'player2') then return end

    if unit.stance == 'Prone' then unit:RemoveStatusEffect("shooting_stance") end
end
...
function OnMsg.UnitStanceChanged(unit)
    ...
    for _, attach in ipairs(attaches) do
        ...
        if parts and parts.Grip and parts.Grip:HasState("folded") then
            local bipod_state = is_holstered or unit.stance ~= "Prone" and
                                    "folded" or unit.stance == "Prone" and
                                    "idle"
...
function OnMsg.GatherCritChanceModifications(attacker, target, action_id,
                                             weapon, data)
    if not weapon then return end
    if not IsKindOf(weapon, "Firearm") then return end
    ...
    if action_id == "BurstFire" or action_id == "AutoFire" or action_id ==
        "RunAndGun" or action_id == "MGBurstFire" or action_id == "GrizzlyPerk" or
        action_id == "BuckshotBurst" then

        data.crit_chance = MulDivRound(data.crit_chance,
                                       const.Combat.BurstFireCriticalChanceMul,
                                       100)
    end
    ----------
...
function OnMsg.GatherCTHModifications(attacker, cth_id, action_id, target,
                                      weapon1, weapon2, data)
    if cth_id == "SameTarget" then
        if data.enabled then data.mod_add = data.mod_add - 8 end
    end
end
...
function OnMsg.GatherCritChanceModifications(attacker, target, action_id,
    ...
    if not (side == 'player1' or side == 'player2') then
        data.crit_chance = MulDivRound(data.crit_chance,
                                       (const.Combat.R_AI_critmul or 100), 100)
    end
end

function OnMsg.GatherDamageModifications(attacker, target, action_id, self,
...
function OnMsg.GatherDamageModifications(attacker, target, action_id, self,
                                         mod_attack_args, mod_hit_data, data)
    local weapon = data.weapon
    if not weapon or not IsKindOf(weapon, "Firearm") then return end
...
function OnMsg.IGIModeChanged(self, new_mode)
    ...
    if IsKindOf(unit, "Unit") then unit:RecalcUIActions(true) end
end
...
function OnMsg.GatherDamageModifications(attacker, target, action_id, self,
    ...
    if weapon then
        if IsKindOf(weapon, "EndlessKnives") then
            ...
            data.base_damage = MulDivRound(data.base_damage, 100 + damage_mul,
                                           100)
        end

        if IsKindOf(weapon, "Unarmed") and HasPerk(attacker, "MartialArts") and
            attacker.species == "Human" then
...
function OnMsg.CombatActionEnd(unit)
    ...
    if unit.action_command == "RunAndGun" or unit.action_command ==
        "RecklessAssault" or unit.action_command == "HundredKnives" or
        unit.action_command == "Sprint" then
    ...
    if unit.Mobile_aimed then unit.Mobile_aimed = nil end
...
function debug_component(w)
    if w:HasComponent("IgnoreInTheDark") then return true end
    return false
end
    ...
    for _, unit in ipairs(g_Units) do
        ...
        for i, w in ipairs(wep) do
            ...
            if not valid then
                ...
                print("trying to remove bugged component in unit:",
                      unit.unitdatadef_id)
...
function OnMsg.EnterSector() debug_weapon_component() end
...
function OnMsg.DamageDone(attacker, target, damage)
    ...
    if targetIsUnit and target:HasStatusEffect('shooting_stance') then
        ...
        if overwatch and overwatch.permanent then return end
...
function OnMsg.EnterSector()
    for _, unit in ipairs(g_Units) do unit.auto_face = false end
end
