...
function rat_GetEnglishVanillaTable(id)
	...
	if csv == {} then
		assert(true, "RATMOD - Could not find english table at " .. path .. "for function at T_ID_enforcement.lua")
		return csv
	end
	local return_Table = {}
	...
	for filepath, table_ in pairs(TID_table) do
		...
		for k,v in pairs(table_) do
			if english_table[k] and (english_table[k] == v) then -- wont export vanilla text as it will already have translations
				...
				table.insert(exportable_table, {id = k, text = v, translation ="", context = "rat_" .. filepath .. "_" .. count, voice = ""})
				count = count +1
			end
		end
	end
