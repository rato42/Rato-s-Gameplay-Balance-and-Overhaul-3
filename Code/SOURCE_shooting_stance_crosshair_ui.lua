...
function redefine_crosshairUI_function()
	function CrosshairUI:UpdateAim()
		...
		local invalidCache = not cached_results or cached_results.aim ~= self.aim or cached_results.ap ~=
							                     attacker.ActionPoints or cached_results.free_move_ap ~= attacker.free_move_ap
		...
		if invalidCache then
			...
			elseif self.targetPart == Presets.TargetBodyPart.Default.BlindFire or self.targetPart ==
								Presets.TargetBodyPart.Default.InCover then -- No longer valid fake bodypart
		...
		if self.idAPCostText then
			...
			local ap_extra = unit:GetApExtra(unit, args and args.target or false, weapon, args and args.aim or 0, action)
			...
			if g_Combat then
				self.idAPCostText:SetText(T {
					prep .. "<apn(apCost)><style CrosshairAPTotal><valign bottom -2>/<apn(unitAp)> AP</style>" .. aim_pen_string,
					apCost = apCost,
					unitAp = unitAp,
				...
				self.idAPCostText:SetText(T {
					prep .. "<apn(apCost)><style CrosshairAPTotal><valign bottom -2> AP</style>" .. aim_pen_string,
					apCost = apCost,
					aim_pen_string = aim_pen_string,
				})
			end
