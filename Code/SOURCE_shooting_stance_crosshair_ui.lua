		...
		if self.idAPCostText then
			...
			if g_Combat then
				self.idAPCostText:SetText(
					T{prep .. "<apn(apCost)><style CrosshairAPTotal><valign bottom -2>/<apn(unitAp)> AP</style>", apCost = apCost, unitAp = unitAp}
				)
			else
				self.idAPCostText:SetText(T{prep .. "<apn(apCost)><style CrosshairAPTotal><valign bottom -2> AP</style>", apCost = apCost})
			end
			------------------
			if self.aim ~= 0 then
