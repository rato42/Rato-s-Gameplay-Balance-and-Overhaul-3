function rat_combat_actions()
	...
	CombatActions.SingleShot.GetAPCost = function(self, unit, args)
		...
		local ap_extra = unit:GetApExtra(unit, args and args.target or false, weapon, args and args.aim or 0, action) or 0	
		...
		return unit:GetAttackAPCost(self, weapon1, false, args and args.aim or 0, ap_delta)+ ap_extra + cycling_ap or -1
	end
	...
	CombatActions.DualShot.GetAPCost = function(self, unit, args)
		...
		local action = self
		local ap_extra1 = unit:GetApExtra(unit, args and args.target or false, weapon, args and args.aim or 0, action)	
		weapon = weapon2
		local ap_extra2 = unit:GetApExtra(unit, args and args.target or false, weapon, args and args.aim or 0, action)	
		...
		return Max(unit:GetAttackAPCost(self, weapon1, weapon1.ShootAP, aim, ap1), unit:GetAttackAPCost(self, weapon2, weapon2.ShootAP, aim, ap2)) + ap_extra + twohand_cost + cycling_total
	...
	CombatActions.BurstFire.GetAPCost = function(self, unit, args)
		...
		if self.CostBasedOnWeapon then
			...
			return weapon and (unit:GetAttackAPCost(self, weapon, nil, args and args.aim or 0) + ap_delta) + ap_extra or -1
		end
	...
	CombatActions.PinDown.GetAPCost = function(self,unit,args) 	
		...
		if HasPerk(unit, "shooting_stance") then
			ap_extra = unit:GetApExtra(unit, args and args.target or false, weapon, args and args.aim or 0, self) or 0		
		end
	...
	CombatActions.SingleShot.Description =  T(585854196899, "Cheap attack that conserves ammo. Has bonus critical chance based on <em>Marksmanship</em> and <em>Dexterity</em> when <em>aimed</em>.")
	...
	CombatActions.AutoFire.Description = T(373274572555, "Shoots a hail of <em><bullets> bullets</em> and inflict <GameTerm('Suppressed')> even on miss when the enemy is in weapon range. Has <em>recoil</em> penalty based on <em>Strength</em>. Maximum <em>aim</em> level reduced")
	CombatActions.DualShot.Description = T(364947777453, "The Dual Shot attack produces a Basic Attack from each gun. Maximum <em>aim</em> level is reduced. Has a penalty based on <em>Dexterity</em>.")
	CombatActions.MGSetup.Description = T(564696256945, "Focus on a cone-shaped area, immobilizing yourself and going <em>prone</em>. You can only shoot enemies inside that cone. Accuracy is increased and enemies will provoke <em>interrupt</em> attacks with actions inside the cone (even if your AP are spent). <em>Interrupt</em> attacks have bonus accuracy.")
	CombatActions.Overwatch.Description= T(226634284341, "<em>Spends all AP</em>. Any targets who move or shoot in the overwatch area will provoke <GameTerm('Interrupt')> <em>attacks</em>. Accuracy is influenced by <em>Reflex</em> (Dex + Agi).")
	...
	CombatActions.RecklessAssault.GetActionDescription = function (self, units)
		...
		local weapon = self:GetAttackWeapons(unit) --or unit:GetActiveWeapons()
		local DisplayMoveAP = rat_getMobileshot_moveAP(self,unit,weapon)
		local description = T(317545478698, "Make a longer <em>Run and Gun</em>, firing more shots. Move to a new position, using up to <em>" .. DisplayMoveAP .. " Move AP</em>. Smiley will be <em>Out of Breath</em> after use. Can't be used when <em>Out of Breath</em>.")
	
		local args = false
		local cost, aimed = self.GetAPCost(self, unit, args)
		
		if aimed then
			description = T(958156855386, "Make a longer <em>Run and Gun</em>, firing more shots. Move to a new position, using up to <em>" .. DisplayMoveAP .. " Move AP</em>. Smiley will be <em>Out of Breath (2 stacks)</em> after use. Can't be used when <em>Out of Breath</em>.\n\nEach shot has <em>1 aim level</em>, but <em>aim accuracy is reduced by half</em>.")
		end
	...
	CombatActions.HundredKnives.GetActionDescription = function (self, units)
		...
		local weapon = self:GetAttackWeapons(unit) --or unit:GetActiveWeapons()
		local DisplayMoveAP = rat_getMobileshot_moveAP(self,unit,weapon)
		local description = T(248617882285, "Move to another position using up to <em>" .. DisplayMoveAP .. " Move AP</em>, while <em>throwing knives</em> at nearby enemies.")
