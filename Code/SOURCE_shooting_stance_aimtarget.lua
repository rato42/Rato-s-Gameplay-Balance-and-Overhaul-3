function Unit:AimTarget(attack_args, attack_results, prepare_to_attack)
    ...
    ------------
    local aim_rotate_cooldown_times = {Standing = 250, Crouch = 500, Prone = 700}
    -------------------------
    ...
    local prepared_attack = attack_args.opportunity_attack_type == "PinDown" or
    ...
    local rotate_to_target = prepare_to_attack or IsValid(attack_args.target) and
                                 IsKindOf(attack_args.target, "Unit")
    ...
    if quick_play then
        if not self.return_pos and not IsCloser2D(self, lof_data.step_pos, const.SlabSizeX / 2) and
            not attack_args.circular_overwatch then
    ...
    if rotate_to_target then
        ...
        while true do
            ...
            while not IsCloser2D(self, lof_data.step_pos, const.SlabSizeX / 2) do
                ...
                if self:ReturnToCover(prefix) then
                    ...
                    if not rotate or self.command ~= "AimIdle" then
                        ...
                        local cover_side = AngleDiff(step_to_target, angle) < 0 and "Left" or
                                               "Right"
                        ...
                        if IsValidAnim(self, anim) then
                            ...
                            local msg = string.format('Missing animation "%s" for "%s"', anim,
                                                      self.unitdatadef_id)
                ...
                lof_idx = table.find(attack_args.lof, "target_spot_group",
                                     attack_args.target_spot_group or "Torso")
            ...
            if self.command ~= "AimIdle" then
                if not attack_args.opportunity_attack or abs(AngleDiff(angle, start_angle)) > 45 *
                    60 then
                    self:AnimatedRotation(angle, aim_anim)
                end
                break
            ...
            if not rotate_cooldown_disable then
                if not self.aim_rotate_last_angle or
                    abs(AngleDiff(angle, self.aim_rotate_last_angle)) > max_deviation_angle then
                    self.aim_rotate_last_angle = angle
                    self.aim_rotate_cooldown_time = GameTime() +
                                                        (aim_rotate_cooldown_times[stance] or 1000)
                    break
                end
            ...
            if not self:HasStatusEffect("shooting_stance") then ------will not change orientation
                ...
                if abs(angle_diff) > 150 * 60 then
                    ...
                    self:SetState(rotate_anim, const.eKeepComponentTargets,
                                  Presets.ConstDef.Animation.BlendTimeRotateOnSpot.value)
