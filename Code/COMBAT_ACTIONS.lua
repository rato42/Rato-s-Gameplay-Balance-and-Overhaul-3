function rat_combat_actions()
    ...
    CombatActions.SingleShot.GetAPCost =
        function(self, unit, args)
            ...
            local ap_extra =
                unit:GetApExtra(unit, args and args.target or false, weapon,
            ...
            return unit:GetAttackAPCost(self, weapon1, false,
                                        args and args.aim or 0, ap_delta) +
                       ap_extra + cycling_ap or -1
        end

    CombatActions.SingleShot.GetUIState =
        function(self, units, args)
            ...
            -------------
            if (IsKindOf(attackWep, "PapovkaSKS_1") or
                IsKindOf(attackWep, "Papovka2SKS_1") or
                ...
                if subwep and subwep.Mountfront then
                    ...
                    if mountFrontAmmo and mountFrontAmmo.Amount and
                        mountFrontAmmo.Amount > 0 then
                        return "disabled", T(758325374611,
                                             "<color AmmoAPColor>Grenade loaded</color>")
                    end
                end
            end
            -------------
    ...
    CombatActions.ChangeWeapon.GetAPCost =
        function(self, unit, args)
            ...
            for _, weapon in ipairs(weapons) do
                if IsKindOfClasses(weapon, "Firearm") then
                    ...
                    if not IsKindOfClasses(weapon, "Pistol", "Revolver") or
                        weapon.pistol_swap then
                        free_swap = false
                    end
    ...
    CombatActions.DualShot.GetAPCost = function(self, unit, args)
        ...
        local action = self
        local ap_extra1 = unit:GetApExtra(unit, args and args.target or false,
                                          weapon, args and args.aim or 0, action)
        weapon = weapon2
        local ap_extra2 = unit:GetApExtra(unit, args and args.target or false,
                                          weapon, args and args.aim or 0, action)
        ...
        return Max(
                   unit:GetAttackAPCost(self, weapon1, weapon1.ShootAP, aim, ap1),
                   unit:GetAttackAPCost(self, weapon2, weapon2.ShootAP, aim, ap2)) +
                   ap_extra + twohand_cost + cycling_total

    end

    CombatActions.BurstFire.GetActionResults =
        function(self, unit, args)
            local args = table.copy(args)

            args.weapon = self:GetAttackWeapons(unit, args)
            args.num_shots = args.num_shots or args.weapon and
                                 args.weapon:GetAutofireShots(self)
            args.multishot = true
            ...
            local results =
                attack_args.weapon:GetAttackResults(self, attack_args)
            return results, attack_args

        end

    CombatActions.SingleShot.GetActionResults =
        function(self, unit, args)
            --------------
            if unit.Mobile_aimed then
                args.aim = 1
    ...
    CombatActions.BurstFire.GetUIState =
        function(self, units, args)
            local unit = units[1]
            local weapon = self:GetAttackWeapons(unit, args)
            ...
            if (IsKindOf(attackWep, "PapovkaSKS_1") or
                ...
                if subwep and subwep.Mountfront then
                    ...
                    if mountFrontAmmo and mountFrontAmmo.Amount and
                        mountFrontAmmo.Amount > 0 then
                        return "disabled", T(163779727424,
                                             "<color AmmoAPColor>Grenade loaded</color>")
                    end
                end
            end
            ...
            if IsKindOf(weapon, "AR15") and
                not weapon:HasComponent("Enable_BurstFire") then
                -- print("no burst")
    ...
    CombatActions.BurstFire.GetAPCost = function(self, unit, args)
        ...
        if self.CostBasedOnWeapon then
            local weapon = self:GetAttackWeapons(unit, args)
            local action = self
            local ap_extra = unit:GetApExtra(unit,
                                             args and args.target or false,
                                             weapon, args and args.aim or 0,
                                             action) or 0
            ...
            return weapon and
                       (unit:GetAttackAPCost(self, weapon, nil,
                                             args and args.aim or 0) + ap_delta) +
                       ap_extra or -1
        end
    ...
    CombatActions.MGBurstFire.GetAPCost =
        function(self, unit, args)
            ...
            local cost = unit:GetAttackAPCost(self, weapon, nil,
                                              args and args.aim or 0)
            cost = cost + ap_extra + ap_delta
            return cost
        end

    CombatActions.GrizzlyPerk.GetAPCost =
        function(self, unit, args)
            if self.CostBasedOnWeapon then
                local weapon = self:GetAttackWeapons(unit, args)
                local action = self
                ...
                return weapon and
                           (unit:GetAttackAPCost(self, weapon, nil,
                                                 args and args.aim or 0,
                                                 ap_delta) + ap_extra) or -1
            end
            return self.ActionPoints
        end

    CombatActions.BuckshotBurst.ActionPointDelta = 2000
    CombatActions.BuckshotBurst.GetAPCost =
        function(self, unit, args)
            ...
            return (unit:GetAttackAPCost(self, weapon1, false,
                                         args and args.aim or 0) + ap_extra +
                       ap_delta) or -1
        end
    ...
    CombatActions.Buckshot.GetAPCost = function(self, unit, args)
        ...
        local ap_extra = unit:GetApExtra(unit, args and args.target or false,
                                         weapon, args and args.aim or 0, action) or
                             0

        return
            unit:GetAttackAPCost(self, weapon1, false, args and args.aim or 0) +
                ap_extra or -1
    end

    CombatActions.DoubleBarrel.GetAPCost =
        function(self, unit, args)
            ...
            local cost = unit:GetAttackAPCost(self, weapon1, false,
                                              args and args.aim or 0) + ap_extra
    ...
    CombatActions.PinDown.GetUIState = function(self, units, args)
        ...
        local weapon = self:GetAttackWeapons(unit, args)
        if not weapon or
            (weapon.PreparedAttackType ~= "Pin Down" and
                weapon.PreparedAttackType ~= "Both") then
            return "hidden"
        end

        if (IsKindOf(weapon, "PapovkaSKS_1") or
            IsKindOf(weapon, "Papovka2SKS_1")) then
            local subwep = weapon.subweapons
            if subwep and subwep.Mountfront then
                local mountFrontAmmo = subwep.Mountfront.ammo
                if mountFrontAmmo and mountFrontAmmo.Amount and
                    mountFrontAmmo.Amount > 0 then
                    return "disabled", T(676922692594,
                                         "<color AmmoAPColor>Grenade loaded</color>")
                end
            end
        end
    ...
    CombatActions.PinDown.GetAPCost = function(self, unit, args)
        ...
        if HasPerk(unit, "shooting_stance") then
            ap_extra = unit:GetApExtra(unit, args and args.target or false,
                                       weapon, args and args.aim or 0, self) or
                           0
        end
        ...
        if HasPerk(unit, "HawksEye") then
            ap = CharacterEffectDefs.HawksEye:ResolveValue(
                     "pindownCostOverwrite") * const.Scale.AP
        end
    ...
    CombatActions.Overwatch.GetAPCost = function(self, unit, args)
        ...
        if not weapon or
            (weapon.PreparedAttackType ~= "Overwatch" and
                weapon.PreparedAttackType ~= "Both") then
            return -1
        end
        ...
        if not HasPerk(unit, "shooting_stance") then
            param = "stance"
            stance = unit:GetApExtra(unit, args and args.target or false,
                                     weapon, args and args.aim or 0, action,
                                     param) or 0
        ...
        local atk_cost = unit:GetAttackAPCost(unit, weapon, nil,
                                              args and args.aim or 0) + AP_delta
        ...
        return Max(unit:GetUIActionPoints(), atk_cost + stance),
               atk_cost + stance
    ...
    CombatActions.Overwatch.GetUIState =
        function(self, units, args)
            local unit = units[1]
    ...
    CombatActions.Overwatch.GetActionDescription =
        function(self, units, args)
            local unit = units[1]
            local apply, value = Presets.ChanceToHitModifier.Default
                                     .OpportunityAttack:CalcValue(unit, nil,
                                                                  nil, nil, nil,
                                                                  nil, nil, nil,
                                                                  true)
            ...
            descr = g_Combat and self.Description or
                        CombatActions.ExplorationOverwatch.Description
            ...
            if action and
                (action.id == "Buckshot" or action.id == "DoubleBarrel") then
    ...
    CombatActions.Overwatch.GetActionDisplayName =
        function(self, units)

            local name = g_Combat and self.DisplayName or
                             CombatActions.ExplorationOverwatch.DisplayName
            local unit = units[1]
    ...
    CombatActions.MGRotate.GetAPCost = function(self, unit, args)
        ...
        local attacker = unit
        local walkable = true
        local target = GetCursorPos(walkable)
        local weapon = attacker:GetActiveWeapons()
        local rotate_ap = ShootingConeAngle(self, attacker, weapon, target)
    ...
    CombatActions.Attack.GetUIState = function(self, units, args)
        ...
        ---------------------------------------------------
        local attackWep = args and args.weapon or weapon or
        ...
        if (IsKindOf(attackWep, "PapovkaSKS_1") or
            ...
            if subwep and subwep.Mountfront then
                ...
                if mountFrontAmmo and mountFrontAmmo.Amount and
                    mountFrontAmmo.Amount > 0 then
                    return "disabled", T(188987732816,
                                         "<color AmmoAPColor>Grenade loaded</color>")
                end
            end
        end
    ...
    CombatActions.RecklessAssault.GetAimParams =
        function(self, unit, weapon)
            ...
            elseif self.AimType == "mobile" then
                ...
                if IsKindOf(weapon, "SubmachineGun") and
                    weapon:HasComponent("Enable_RunAndGun") then
                    -- print("sub")
                    shots = shots + 1
    ...
    CombatActions.RecklessAssault.GetUIState =
        function(self, units, args)
            if not g_Combat then
            ...
            if actions["RunAndGun"] ~= nil then
                ...
                -- print("ui", ui_status)
                if ui_status and
                    (ui_status == "Hidden" or ui_status == "disabled") then
                    local p, reason = CombatActions.RunAndGun.GetUIState(self,
                                                                         units,
                                                                         args)
                    return "disabled", reason
                end
            else
                return "disabled", T(565263742146,
                                     "<color AmmoAPColor>Weapon not capable of burst</color>")
            end

            if unit:HasStatusEffect("R_outofbreath") then
                return "disabled", T(865874231348,
                                     "<color AmmoAPColor>Out of Breath</color>")
            end
    ...
    CombatActions.RunAndGun.GetUIState =
        function(self, units, args)
            if not g_Combat then
                return "disabled", AttackDisableReasons.CombatOnly
            end
            ...
            if unit:HasStatusEffect("R_outofbreath") then
                return "disabled", T(536142745929,
                                     "<color AmmoAPColor>Out of Breath</color>")
    ...
    CombatActions.RunAndGun.GetAPCost = function(self, unit, args)
        ...
        -- local ap_extra = (weapon.APStance + 1 )*const.Scale.AP --unit:GetApExtra(unit, args and args.target or false, weapon, args and args.aim or 0, action) or 0	
    ...
    CombatActions.RunAndGun.GetActionResults =
        function(self, unit, args)
            ...
            args.num_shots = weapon and weapon:GetAutofireShots("BurstFire") or
                                 CombatActions.BurstFire:ResolveValue(
                                     "num_shots")
            args.multishot = true
    ...
    CombatActions.RunAndGun.GetActionDescription =
        function(self, units)
            ...
            local penalty =
                Presets.ChanceToHitModifier.Default.RunAndGun:ResolveValue(
                    "Penalty")
            ...
            local description = T(336535188124,
                                  "Move to a new position, using up to <em>" ..
                                      DisplayMoveAP ..
            ...
            if unit.stance ~= "Standing" then
                description = description ..
                                  T(801805830695,
                                    "<newline>You will end up in Standing stance.")
            end

            return description
        end

    CombatActions.RunAndGun.GetActionDisplayName =
        function(self, units)
    ...
    CombatActions.MobileShot.GetAPCost =
        function(self, unit, args)
    ...
    CombatActions.MobileShot.GetActionResults =
        function(self, unit, args)
            local cost, aimed = self.GetAPCost(self, unit, args)
    ...
    CombatActions.MobileShot.GetUIState =
        function(self, units, args)
            if not g_Combat then
                return "disabled", AttackDisableReasons.CombatOnly
            end
            ...
            if (IsKindOf(attackWep, "PapovkaSKS_1") or
                ...
                if subwep and subwep.Mountfront then
                    ...
                    if mountFrontAmmo and mountFrontAmmo.Amount and
                        mountFrontAmmo.Amount > 0 then
                        return "disabled", T(359627666793,
                                             "<color AmmoAPColor>Grenade loaded</color>")
                    end
                end
            end
            ...
            if unit:HasStatusEffect("R_outofbreath") then
                return "disabled", T(545341281514,
                                     "<color AmmoAPColor>Out of Breath</color>")
    ...
    CombatActions.MobileShot.GetActionDisplayName =
        function(self, units)
            local name = self.DisplayName
            local unit = units[1]
            local args = false
    ...
    CombatActions.MobileShot.GetActionDescription =
        function(self, units)
            local description = self.Description
            local unit = units and units[1]
            if not unit then
    ...
    CombatActions.RecklessAssault.GetActionDescription =
        function(self, units)
    ...
    CombatActions.RecklessAssault.GetAPCost =
        function(self, unit, args)
    ...
    CombatActions.RecklessAssault.GetActionDisplayName =
        function(self, units)
    ...
    CombatActions.HundredKnives.GetAPCost =
        function(self, unit, args)
            local weapon = self:GetAttackWeapons(unit, args)
            local cost = unit:GetAttackAPCost(self, self:GetAttackWeapons(unit,
                                                                          args),
                                              unit.ActionPoint)
    ...
    CombatActions.HundredKnives.GetUIState =
        function(self, units, args)
            ...
            if unit:HasStatusEffect("R_outofbreath") then
                return "disabled", T(956881287967,
                                     "<color AmmoAPColor>Out of Breath</color>")
    ...
    CombatActions.HundredKnives.GetAimParams =
        function(self, unit, weapon)
            if self.AimType == "cone" then
                return weapon:GetAreaAttackParams(self.id, unit)
            elseif self.AimType == "mobile" then
    ...
    CombatActions.HundredKnives.GetActionDescription =
        function(self, units)
            ...
            local description = T(248617882285,
                                  "Move to another position using up to <em>" ..
                                      DisplayMoveAP ..
    ...
    CombatActions.MobileShot.GetAimParams =
        function(self, unit, weapon)
            if self.AimType == "cone" then
                return weapon:GetAreaAttackParams(self.id, unit)
            elseif self.AimType == "mobile" then
    ...
    CombatActions.RunAndGun.GetActionDamage =
        function(self, unit, target, args)
            local weapon = self:GetAttackWeapons(unit, args)
            -- print(self)
            if not weapon then
                return 0
            ...
            elseif (IsKindOf(weapon, "AssaultRifle") or
                IsKindOf(weapon, "SniperRifle") or
            ...
            num_shots =
                "<style CrosshairAPTotal><color PDABrowserTextHighlight>" ..
                    num_shots .. "X " .. "</style></color>"
            return T(num_shots .. num_shots_real .. "X" .. damage)

        end

    CombatActions.MobileShot.GetActionDamage =
        function(self, unit, target, args)
            local weapon = self:GetAttackWeapons(unit, args)
    ...
    CombatActions.RunAndGun.GetAimParams =
        function(self, unit, weapon)
            if self.AimType == "cone" then
                return weapon:GetAreaAttackParams(self.id, unit)
            elseif self.AimType == "mobile" then
    ...
    CombatActions.MGBurstFire.GetActionDamage =
        function(self, unit, target, args)
            local weapon =
                args and args.weapon or self:GetAttackWeapons(unit, args)
            if not weapon then
                return 0
            end
    ...
    CombatActions.MGBurstFire.GetActionResults =
        function(self, unit, args)
            local args = table.copy(args)
...
function rat_MGSetup_getap()
    CombatActions.MGSetup.GetAPCost = function(self, unit, args)
        ...
        if HasPerk(unit, "HeavyWeaponsTraining") then
            local effect = unit:GetStatusEffect("HeavyWeaponsTraining")
            local reduction = effect:ResolveValue("ap_cost_reduction") *
                                  const.Scale.AP
            local minCost = effect:ResolveValue("min_ap_cost") * const.Scale.AP
